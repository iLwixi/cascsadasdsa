...
async function handleFlagReaction(emoji, message, user) {
  ...
  if (remaining > 0) {
    return message.channel.safeSend(`${user} Beklemelisin ${timeformat(remaining)} Tekrar çevirmeden önce!`, 5);
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setAuthor({ name: `Çeviri ${src}` })
    .setDescription(desc)
    .setFooter({
      text: `Tarafından talep edildi ${user.tag}`,
