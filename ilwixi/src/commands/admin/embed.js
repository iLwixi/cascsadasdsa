...
module.exports = {
  ...
  async messageRun(message, args) {
    ...
    if (!channel) return message.reply("Lütfen geçerli bir kanal belirtin");
    if (channel.type !== ChannelType.GuildText) return message.reply("Lütfen geçerli bir kanal belirtin");
    if (!channel.canSendEmbeds()) {
      return message.reply("Bu kanala embed gönderme iznim yok");
    }
  ...
  async interactionRun(interaction) {
    ...
    if (!channel.canSendEmbeds()) {
      return interaction.followUp("Bu kanala embed gönderme iznim yok");
    }
...
async function embedSetup(channel, member) {
  const sentMsg = await channel.send({
    content: "Başlamak için aşağıdaki düğmeye tıklayın",
    components: [
  ...
  if (!btnInteraction) return sentMsg.edit({ content: "Yanıt alınmadı", components: [] });
  ...
  if (!modal) return sentMsg.edit({ content: "Yanıt alınamadı, kurulum iptal ediliyor", components: [] });
  ...
  if (!title && !author && !description && !footer)
    return sentMsg.edit({ content: "Boş bir embed gönderemezsiniz!", components: [] });
  ...
  await sentMsg.edit({
    content: "Lütfen aşağıdaki butonları kullanarak alanları ekleyin. İşiniz bittiğinde bitti'yi tıklayın.",
    embeds: [embed],
  ...
  collector.on("collect", async (interaction) => {
    if (interaction.customId === "EMBED_FIELD_ADD") {
      ...
      modal.reply({ content: "Alan eklendi", ephemeral: true }).catch((ex) => {});
    ...
    // remove field
      ...
      if (fields) {
        fields.pop();
        embed.setFields(fields);
        interaction.reply({ content: "Alan silindi", ephemeral: true });
      } else {
        interaction.reply({ content: "Kaldırılacak alan yok", ephemeral: true });
      }
    }
