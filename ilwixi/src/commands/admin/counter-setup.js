...
module.exports = {
  name: "sayac",
  description: "bu sunucuda sayaç kanalını ayarla",
  category: "ADMIN",
  ...
  async messageRun(message, args, data) {
    ...
    if (!type || !["USERS", "MEMBERS", "BOTS"].includes(type)) {
      return message.safeReply("Yanlış argümanlar geçti! Sayaç türleri: `users/members/bots`");
    }
    if (args.length < 2) return message.safeReply("Yanlış kullanım!İsim vermedin");
    args.shift();
...
async function setupCounter(guild, type, name, settings) {
  ...
  return "Yapılandırma kaydedildi!Counter Kanal Oluşturuldu";
}
