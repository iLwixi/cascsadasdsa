    ...
    subcommands: [
      {
        ...
        description: "Yeni bir hediye ayarlayın",
      },
      {
        trigger: "pause <messageId>",
        description: "bir hediyeyi duraklat",
      },
      {
        trigger: "resume <messageId>",
        description: "duraklatılmış bir hediye devam et",
      },
      {
        trigger: "end <messageId>",
        description: "bir hediye son vermek",
      },
      {
        trigger: "reroll <messageId>",
        description: "çekilişi kazanan kişiyi tekrardan belirler",
      },
      {
        trigger: "list",
        description: "Tüm hediyeleri listeleyin",
      },
      {
        trigger: "edit <messageId>",
        description: "Bir hediye düzenleyin",
      },
    ],
  },
  slashCommand: {
...
async function runModalSetup({ member, channel, guild }, targetCh) {
  ...
  if (!targetCh) return channel.safeSend("Hediye kurulumu iptal edildi.Bir kanaldan bahsetmedin");
  if (!targetCh.type === ChannelType.GuildText && !targetCh.permissionsFor(guild.members.me).has(SETUP_PERMS)) {
    return channel.safeSend(
      `Hediye kurulumu iptal edildi.\nihtiyacım var ${parsePermissions(SETUP_PERMS)}  ${targetCh}`
    );
  }
  ...
  const sentMsg = await channel.safeSend({
    content: "Yeni hediye ayarlamak için lütfen aşağıdaki düğmeyi tıklayın",
    ...
    new ModalBuilder({
      ...
      components: [
        ...
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId("roles")
            .setLabel("Hediyeye katılabilecek rolid")
            .setStyle(TextInputStyle.Short)
            .setRequired(false)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId("host")
            .setLabel("Hediye barındıran kullanıcı kimliği")
            .setStyle(TextInputStyle.Short)
            .setRequired(false)
        ),
      ],
  ...
  await modal.reply("Hediye Ayarlama...");

  // duration
  const duration = ems(modal.fields.getTextInputValue("duration"));
  if (isNaN(duration)) return modal.editReply("Kurulum iptal edildi.Geçerli bir süre belirtmediniz");
  ...
  if (isNaN(winners)) return modal.editReply("Kurulum iptal edildi.Geçerli bir kazanan sayısı belirtmediniz");
  ...
  if (hostId) {
    try {
      ...
      return modal.editReply("Kurulum iptal edildi.Ana bilgisayar için geçerli bir kullanıcı kimliği sağlamanız gerekir");
    }
  }
...
async function runModalEdit(message, messageId) {
  ...
  const sentMsg = await channel.send({
    content: "Hediyeyi düzenlemek için lütfen aşağıdaki düğmeyi tıklayın",
    components: [buttonRow],
  });
  ...
  if (!btnInteraction) return sentMsg.edit({ content: "Yanıt alınmadı, güncellemeyi iptal ediyor", components: [] });

  // display modal
  await btnInteraction.showModal(
    new ModalBuilder({
      customId: "giveaway-modalEdit",
      title: "Hediye Güncellemesi",
      components: [
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId("duration")
            .setLabel("Ekleme Süresi")
            .setPlaceholder("1h / 1d / 1w")
            .setStyle(TextInputStyle.Short)
            .setRequired(false)
        ),
  ...
  if (!modal) return sentMsg.edit({ content: "Yanıt alınmadı, güncellemeyi iptal ediyor", components: [] });

  sentMsg.delete().catch(() => {});
  await modal.reply("Hediye güncelleme...");

  // duration
  const addDuration = ems(modal.fields.getTextInputValue("duration"));
  if (isNaN(addDuration)) return modal.editReply("Güncelleme iptal edildi.Geçerli bir ekleme süresi belirtmediniz");
  ...
  if (isNaN(newWinnerCount)) {
    return modal.editReply("Güncelleme iptal edildi.Geçerli bir kazanan sayısı belirtmediniz");
  }
