...
module.exports = {
  name: "rank",
  description: "Bu sunucudaki üyelerin sıralamasını görüntüler",
  cooldown: 5,
  category: "STATS",
...
async function getRank({ guild }, member, settings) {
  ...
  if (!memberStats.xp) return `${user.tag} henüz sıralanmadı!`;
  ...
  if (!response.success) return "Sıralama kartı oluşturulamadı";
