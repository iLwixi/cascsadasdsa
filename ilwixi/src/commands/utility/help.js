...
module.exports = {
  name: "yardım",
  description: "Komut Yardım Menüsü",
  category: "UTILITY",
  ...
  async messageRun(message, args, data) {
    ...
    await message.safeReply("Eşleştirme komutu bulunamadı");
  },
  ...
  async interactionRun(interaction) {
    ...
    // No matching command/category found
    await interaction.followUp("Eşleştirme komutu bulunamadı");
  },
};
...
async function getHelpMenu({ client, guild }) {
  ...
  for (const [k, v] of Object.entries(CommandCategory)) {
    ...
    options.push({
      label: v.name,
      value: k,
      description: `Komutları görüntüle ${v.name} kategori`,
      emoji: v.emoji,
    });
  }
  ...
  const menuRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder("Komut kategorisini seçin")
      .addOptions(options)
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**Benim Hakkımda:**\n" +
        `Merhaba ben ${guild.members.me.displayName}!\n` +
        "Ben sizin için yeterim\n\n" +
        "ZeKe_Hayarni tarafından yapıldım\n\n" +
        `**Beni sunucuna davet et!:** [Tıkla](${client.getInvite()})\n` +
        `**Destek sunucum:** [Katıl](${SUPPORT_SERVER})`
    );
...
function getSlashCategoryEmbeds(client, category) {
  ...
  if (commands.length === 0) {
    const embed = new EmbedBuilder()
      ...
      .setThumbnail(CommandCategory[category]?.image)
      .setAuthor({ name: `${category} Commands` })
      .setDescription("Bu kategoride komut yok");
  ...
  while (commands.length) {
    ...
    toAdd = toAdd.map((cmd) => {
      ...
      return `\`/${cmd.name}\`\n ❯ **Tanım**: ${cmd.description}\n ${
        !subCmds?.length ? "" : `❯ **Alt komutlar [${subCmds?.length}]**: ${subCmdsString}\n`
      } `;
    });
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      .setColor(EMBED_COLORS.BOT_EMBED)
      .setThumbnail(CommandCategory[category]?.image)
      .setAuthor({ name: `${category} Komutlar` })
...
function getMsgCategoryEmbeds(client, category, prefix) {
  ...
  if (category === "IMAGE") {
    ...
    collector +=
      "\n\nBu görüntü komutlarını aşağıdaki biçimlerde kullanabilirsiniz\n" +
      `**${prefix}cmd:** Mesaj Yazarları Avatar'ı görüntü olarak seçer\n` +
      `**${prefix}cmd <@member>:** Seçimler Üyeler Avatar'ı görüntü olarak belirtti\n` +
      `**${prefix}cmd <url>:** Verilen URL'den görüntü seçer\n` +
      `**${prefix}cmd [attachment]:** Seçiciler Ek Resim`;
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `page ${index + 1} of ${arrSplitted.length} | Tip ${prefix}help <komut> Daha fazla komut bilgisi için`,
      });
    arrEmbeds.push(embed);
  });
