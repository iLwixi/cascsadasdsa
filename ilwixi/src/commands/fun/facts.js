...
module.exports = {
  name: "facts",
  description: "rastgele bir hayvan gerçeklerini gösterir ",
  cooldown: 5,
  category: "FUN",
  ...
  async messageRun(message, args) {
    ...
    if (!animals.includes(choice)) {
      return message.safeReply(`Geçersiz hayvan seçildi.Mevcut hayvanlar:\n${animals.join(", ")}`);
    }
...
async function getFact(user, choice) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(fact)
    .setFooter({ text: `Tarafından talep edildi ${user.tag}` });
