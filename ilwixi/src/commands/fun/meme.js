...
module.exports = {
  name: "meme",
  description: "Rastgele Meme Alın",
  category: "FUN",
...
async function getRandomEmbed(choice) {
  try {
    ...
    if (!data.success) {
      return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription("Meme getirilemedi.Tekrar deneyin!");
    }

    const memes = data.data.memes;
    if (!Array.isArray(memes) || memes.length === 0) {
      return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription("Mem bulunamadı.");
    }
